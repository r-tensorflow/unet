...
In this example we will use the `unet` package to create an U-Net model that
could be used to remove the background from images in the Carvana dataset.

The dataset we are goind to use appeared first in the [Carvana Image Masking Challenge](https://www.kaggle.com/c/carvana-image-masking-challenge/overview)
on Kaggle. You can see more information in the [competition page](https://www.kaggle.com/c/carvana-image-masking-challenge/overview).
...
Here are some examples of what you can find in the dataset. On left we can find the
...
Now let's start building building our model. We will use `tfdatasets` to make our
...
The `[` calls wound't be necessary if `tf$image$decode_gif` returned a 3D Tensor like `tf$image$decode_jpeg` does. And if it could read just one color channel as we are only interested if it's black and white.

If you are running this code interactively you can easilly see the output of this
chunk with:
...
The above loaded the images into into an `uint8` Tensor. Which is great for reading 
as it uses less memory. However for modelling we prefer having `float32` Tensors 
and that the values are in the [0,1] range. That's what will will fix now:
...
The images from our dataset are in a pretty high definition (1280x1918) but we will resize them to reduce the computing cost of the model. We are going to resizem them to 128x128. This size is completely arbitrary.
...
We can now map this function into the images:
...
the valdiation dataset, and that what we are going to do.
...
2. `dataset_suffle` to shuffle the dataset
3. `dataset_map(unname)` since Keras needs an unnamed input.
...
called Dice that can be implemente like this:
